    ...
    if len(norm_tweet) >= 15:
        ...
        for k in tweet_map_keys:
            if len(k) >= 15 and (norm_tweet in k or k in norm_tweet): 
                len_diff = abs(len(k) - len(norm_tweet))
        ...
        if best_match_key:
            ...
            for info in target_infos:
                if info["lemma"].lower() == last_predicador or info["roleset"].lower().startswith(last_predicador) or info["lemma"].lower() in last_predicador or last_predicador in info["lemma"].lower():
                    info_matches.append(info)
                    
            if not info_matches and last_predicador:
                roleset_guess = f"{last_predicador}.01"
                new_info = {"lemma": last_predicador, "roleset": roleset_guess, "args": {}}
                target_infos.append(new_info)
                info_matches = [new_info]
                
            if not info_matches:
                info_matches = [target_infos[0]]
                
            argm_col = argm_col.replace('; ARGM', '| ARGM').replace('; argm', '| argm')
            for part in argm_col.split('|'):
                part = part.strip()
                if not part: continue
                # Match "ARGM-XXX" seguido de qualquer separador e o texto
                match = re.match(r'^\s*(ARGM-[A-Z]{3})(?:[-\:=]|\s)*\s*(.*)', part, re.I)
                if not match: continue
                clean_lbl = match.group(1).lower()
                txt = match.group(2).split('(')[0].strip()
...
if __name__ == "__main__":
    ...
    if os.path.basename(base_dir) == "argn e argm":
        root_dir = os.path.dirname(os.path.dirname(base_dir))

    parser = argparse.ArgumentParser(description="Conversor de Anotações JSON/Excel para CoNLL-U Plus")
    ...
    parser.add_argument("--input-dir", default=os.path.join(root_dir, "data", "input_conllu"), help="Pasta com arquivos .conllu de entrada (Padrão: ../data/input_conllu)")
    parser.add_argument("--output-dir", default=os.path.join(root_dir, "output"), help="Pasta de saída (Padrão: ../output)")
    
    args = parser.parse_args()
    
    if not os.path.isdir(args.json_dir):
        print(f"Erro: A pasta '{args.json_dir}' não foi encontrada. Verifique o caminho ou use --json-dir.")
        sys.exit(1)
    if not os.path.isfile(args.excel_path):
        print(f"Erro: O arquivo '{args.excel_path}' não foi encontrado. Verifique o caminho ou use --excel-path.")
        sys.exit(1)
    if not os.path.isdir(args.input_dir):
        print(f"Erro: A pasta '{args.input_dir}' não foi encontrada. Verifique o caminho ou use --input-dir.")
        sys.exit(1)

    input_files = glob.glob(os.path.join(args.input_dir, "*.conllu"))
    if not input_files:
        print(f"Aviso: Nenhum arquivo .conllu encontrado em '{args.input_dir}'.")
        sys.exit(0)

    os.makedirs(args.output_dir, exist_ok=True)

    print("Carregando e indexando JSONs em memória...")
    rolesets, tweet_map = load_rolesets(args.json_dir)
    print("Processando ARGMs e fundindo com o gabarito principal...")
    load_argms(args.excel_path, tweet_map)

    for input_txt in input_files:
        filename = os.path.basename(input_txt)
        nome_base, ext = os.path.splitext(filename)
        output_txt = os.path.join(args.output_dir, f"{nome_base}.conllup")
