...
if __name__ == "__main__":
    ...
    historico_dir = r"D:\OneDrive\Documentos\Ling\IC\Conversao em massa\historico\argn e argm"
    
    os.makedirs(historico_dir, exist_ok=True)
    
    conllu_files = [
        os.path.join(output_dir, "DANTEStocks-dev (conllu plus).conllup"),
        os.path.join(output_dir, "DANTEStocks-test (conllu plus).conllup"),
        os.path.join(output_dir, "DANTEStocks-train (conllu plus).conllup")
    ]
    ...
    expected_data = load_expected_data(json_dir)
    load_argms_into_expected(excel_path, expected_data)
    
    for conllu_file in conllu_files:
        log_print(f"\n--- Analisando arquivo: {os.path.basename(conllu_file)} ---")
        if not os.path.exists(conllu_file):
            log_print("Arquivo não encontrado. Execute o conversor primeiro.")
            continue
    ...
    version = 9
    existing_logs = glob.glob(os.path.join(historico_dir, f"log V{version} *.txt"))
    max_y = 0
    ...
    next_y = max_y + 1
    log_filename = os.path.join(historico_dir, f"log V{version} {next_y}.txt")
